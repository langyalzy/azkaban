...
public class TriggerManager extends EventHandler implements
  ...
  private static final Map<Integer, Trigger> triggerIdMaps = new ConcurrentHashMap<>();
  ...
  @Override
  public List<Trigger> getTriggerUpdates(String searchParam ,  int skip, int num)
  		throws TriggerManagerException {
	  
	  triggerIdMaps.clear();
	List<Trigger> triggers1 = null;
	try {
		triggers1 = this.triggerLoader.loadTriggers(searchParam , skip , num);
      ...
      for (final Trigger t : triggers1) {
    	  triggerIdMaps.put(t.getTriggerId(), t);
      }
     
	  final List<Trigger> triggers = new ArrayList<>();
	    for (final Trigger t : triggerIdMaps.values()) {
	        triggers.add(t);
	    }
