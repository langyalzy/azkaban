...
public class JdbcTriggerImpl implements TriggerLoader {
  ...
  private static final String UPDATE_TRIGGER =
      "UPDATE " + TRIGGER_TABLE_NAME
          + " SET trigger_source=?, modify_time=?, enc_type=?, data=?,project_name=?,project_flow_name=?,cron_expression=? WHERE trigger_id=?";
  ...
  @Override
  public List<Trigger> loadTriggers(String searchParam , int skip , int num) throws TriggerLoaderException {
    logger.info("Loading all triggers from db.");

    final ResultSetHandler<List<Trigger>> handler = new TriggerResultHandler();
    
    StringBuffer where = new StringBuffer();
    if(!StringUtils.isBlank(searchParam) && !searchParam.equals("\001")) {
    	where.append(" where project_name like '%"+searchParam+"%' or project_flow_name like '%"+searchParam+"%' or cron_expression like '%"+searchParam+"%'");
    }
    where.append(" ");
    
    
    try {
      final List<Trigger> triggers = this.dbOperator.query(GET_ALL_TRIGGERS + where.toString()+" LIMIT "+skip + " , "+ num, handler);
      logger.info("Loaded-------------------------------------------------------------------------------------------- " + triggers.size() + " triggers.");
      return triggers;
    } catch (final SQLException ex) {
      throw new TriggerLoaderException("Loading triggers from db failed.", ex);
  ...
  public static void main(String[] args) {
	  ...
	  JsonObject returnData = new JsonParser().parse("{\"triggerCondition\":{\"nextCheckTime\":\"1590723900000\",\"expression\":\"BasicTimeChecker_1.eval()\",\"checkers\":[{\"checkerJson\":{\"nextCheckTime\":\"1590723900000\",\"cronExpression\":\"0 * * ? * *\",\"period\":\"null\",\"skipPastChecks\":\"true\",\"timezone\":\"Asia/Shanghai\",\"firstCheckTime\":\"1590564387421\",\"isRecurring\":\"true\",\"id\":\"BasicTimeChecker_1\",\"type\":\"BasicTimeChecker\"},\"type\":\"BasicTimeChecker\"}]},\"resetOnTrigger\":\"true\",\"triggerId\":\"6\",\"source\":\"SimpleTimeTrigger\",\"resetOnExpire\":\"true\",\"expireActions\":[],\"submitTime\":\"1590564387421\",\"submitUser\":\"azkaban\",\"lastModifyTime\":\"1590723903181\",\"context\":{},\"actions\":[{\"actionJson\":{\"submitUser\":\"azkaban\",\"actionId\":\"executeFlowAction\",\"executionOptions\":{\"failureAction\":\"FINISH_CURRENTLY_RUNNING\",\"memoryCheck\":true,\"queueLevel\":0,\"pipelineExecId\":null,\"concurrentOption\":\"skip\",\"notifyOnFirstFailure\":false,\"notifyOnLastFailure\":false,\"pipelineLevel\":null,\"successEmailsOverride\":false,\"failureEmails\":[],\"disabled\":[],\"flowParameters\":{},\"successEmails\":[],\"mailCreator\":\"default\",\"failureEmailsOverride\":false},\"type\":\"ExecuteFlowAction\",\"projectName\":\"test\",\"projectId\":\"29\",\"flowName\":\"common\"},\"type\":\"ExecuteFlowAction\"}],\"expireCondition\":{\"nextCheckTime\":\"2524608000000\",\"expression\":\"EndTimeChecker_1.eval()\",\"checkers\":[{\"checkerJson\":{\"nextCheckTime\":\"2524608000000\",\"cronExpression\":null,\"period\":\"null\",\"skipPastChecks\":\"false\",\"timezone\":\"Asia/Shanghai\",\"firstCheckTime\":\"1590564387421\",\"isRecurring\":\"false\",\"id\":\"EndTimeChecker_1\",\"type\":\"BasicTimeChecker\"},\"type\":\"BasicTimeChecker\"}]},\"status\":\"READY\",\"info\":{}}").getAsJsonObject();
	  
	  returnData.getAsJsonObject("triggerCondition").getAsJsonArray("checkers").forEach(item->{
		  System.out.println("================");
		  if(item.getAsJsonObject().get("checkerJson") != null) {
			  System.out.println("================"+item.getAsJsonObject().getAsJsonObject("checkerJson").get("cronExpression"));
		  }
		  
	  });
	  returnData.getAsJsonArray("actions").forEach(item->{
		  if(item.getAsJsonObject().get("actionJson") != null) {
			  String projectName = item.getAsJsonObject().getAsJsonObject("actionJson").get("projectName").getAsString();
			  String projectFlowName = item.getAsJsonObject().getAsJsonObject("actionJson").get("flowName").getAsString();
			  System.out.println(projectName + "===" + projectFlowName);
		  }
	  });
  ...
  private void updateTrigger(final Trigger t, final EncodingType encType)
    ...
    Iterator<JsonElement> it = returnData.getAsJsonObject("triggerCondition").getAsJsonArray("checkers").iterator();
    while(it.hasNext()) {
    	JsonElement item = it.next();
    	 if(item.getAsJsonObject().get("checkerJson") != null) {
			  cronExpression = item.getAsJsonObject().getAsJsonObject("checkerJson").get("cronExpression").getAsString();
		 }
    	 
    }
    
    Iterator<JsonElement> its = returnData.getAsJsonArray("actions").iterator();
    while(its.hasNext()) {
    	JsonElement item = its.next();
    	
    	if(item.getAsJsonObject().get("actionJson") != null) {
    		projectName = item.getAsJsonObject().getAsJsonObject("actionJson").get("projectName").getAsString();
    		projectFlowName = item.getAsJsonObject().getAsJsonObject("actionJson").get("flowName").getAsString();
    	}
    ...
    try {
      final int updates = this.dbOperator
          .update(UPDATE_TRIGGER, t.getSource(), t.getLastModifyTime(), encType.getNumVal(), data,projectName,projectFlowName,cronExpression,
              t.getTriggerId());
      if (updates == 0) {
