...
public class TriggerBasedScheduleLoader implements ScheduleLoader {
  ...
  @Override
  public synchronized List<Schedule> loadUpdatedSchedules(String searchParam , int skip , int num)
      throws ScheduleManagerException {
    final List<Trigger> triggers;
    try {
      triggers = this.triggerManager.getTriggerUpdates(searchParam , skip , num);
    } catch (final TriggerManagerException e) {
      e.printStackTrace();
      throw new ScheduleManagerException(e);
    }
